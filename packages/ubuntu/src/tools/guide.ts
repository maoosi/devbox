...
const INSTALL_ONELINER = `curl -fsSL https://raw.githubusercontent.com/maoosi/devbox/main/packages/ubuntu/install.sh | bash`;
...
2. Delete the \`export GH_TOKEN='…'\` line from \`~/.config/devbox/env\`.
...
2. Delete the \`export DOPPLER_TOKEN='…'\` line from \`~/.config/devbox/env\`.
3. Re-run the installer:
...
2. Delete the \`export INFISICAL_TOKEN='…'\` line from \`~/.config/devbox/env\`.
...
- re-prompts for a GitHub PAT with the new scope (revoke the old one in the
  GitHub UI first, then delete the \`export GH_TOKEN='…'\` line from
...
to match the lists in this section.
...
see what your re-run actually changed.
...
- \`~/.config/devbox/aliases.sh\` — Socket Firewall wrappers (when installed).
